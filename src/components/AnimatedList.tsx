...
import type { ReactNode, MouseEventHandler, UIEvent, RefObject, MutableRefObject } from 'react';
...
const AnimatedItem: React.FC<AnimatedItemProps> = ({ children, delay = 0, index, itemRef, onMouseEnter, onClick }) => {
...
interface AnimatedListProps {
  items?: string[];
  itemRefs?: Array<RefObject<HTMLDivElement | null>>;
  ...
  onListReady?: (element: HTMLDivElement) => void;
}
...
const AnimatedList: React.FC<AnimatedListProps> = ({
  ...
  useEffect(() => {
    ...
    if (listContainerRef) {
      listContainerRef.current = listRef.current;
    }
    onListReady?.(listRef.current);
  }, [listContainerRef, onListReady]);
      ...
      >
        {items.map((item, index) => {
          ...
          return (
            ...
            >
              <div className={`p-4 bg-[#111] rounded-lg ${selectedIndex === index ? 'bg-[#222]' : ''} ${resolvedItemClassName}`}>
                {renderItem ? renderItem(item, index, selectedIndex === index) : <p className="text-white m-0">{item}</p>}
              </div>
            </AnimatedItem>
          );
        })}
